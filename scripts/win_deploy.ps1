...
function Invoke-Deploy {
    ...
    $modelMap = @{
        "t-lite-2.1"  = @{ template="qwen";   prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — начинай с <think>твои рассуждения здесь</think>, потом дай ответ. В простых вопросах отвечай сразу без тегов." }
        "t-pro-2.0"   = @{ template="qwen";   prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — начинай с <think>твои рассуждения здесь</think>, потом дай ответ. В простых вопросах отвечай сразу без тегов." }
        "qvikhr-4b"   = @{ template="qwen";   prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — начинай с <think>твои рассуждения здесь</think>, потом дай ответ. В простых вопросах отвечай сразу без тегов." }
        "qvikhr-1b"   = @{ template="qwen";   prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — начинай с <think>твои рассуждения здесь</think>, потом дай ответ. В простых вопросах отвечай сразу без тегов." }

        "saiga-gem12" = @{ template="gemma";  prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — используй <think>твои рассуждения здесь</think> перед ответом. В обычных вопросах отвечай сразу без тегов." }

        "saiga-nem12" = @{ template="llama3"; prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — начинай с <think>твои рассуждения здесь</think>, потом дай ответ. В простых вопросах отвечай сразу без тегов." }
        "yagpt-8b"    = @{ template="llama3"; prompt="Ты полезный помощник. Отвечай только по-русски. Если попросят подумать шаг за шагом или объяснить подробно — начинай с <think>твои рассуждения здесь</think>, потом дай ответ. В простых вопросах отвечай сразу без тегов." }

        "kodify-2b"   = @{ template="qwen";   prompt="Ты полезный кодер-помощник. Отвечай только по-русски. Если попросят подумать — используй <think>твои рассуждения здесь</think>." }
    }

    $baseName = ($candidate.name -split "-q[0-9]")[0]
    $cfg = if ($modelMap.ContainsKey($baseName)) { $modelMap[$baseName] } else { $modelMap["t-lite-2.1"] }
